...
public class IntervalSignal  extends Signal<IntervalVector> {
	...
	public IntervalSignal(int k, List<VarDecl> inputs, Model m, IntervalVector span) {
		for(int time = 0; time < k; time++){
			...
			for(VarDecl var : inputs){
				...
				if(v == null){
					JFuzzInterval jfi = span.get(tname);
					iv.put(tname, jfi);
				}
				else{
					...
					JFuzzInterval jfi = new JFuzzInterval((NamedType) var.type, lowerBound, upperBound);
	...
	public void put(int time, TypedName key, JFuzzInterval value) {
	...
	private BigFraction biasedRandomValue(JFuzzInterval x, BigFraction target) {
		...
		if (x instanceof RealValue) {
			...
			if(! lowEP.isFinite()){
				if (! val.getHigh().isFinite()){
					...
					return JFuzzInterval.defaultLow(nt);
				}
				else{
		...
		if (x instanceof RealValue) {
			...
			if(! hiEP.isFinite()){
				if (! val.getLow().isFinite()){
					NamedType nt = (isRealEP ? NamedType.REAL : NamedType.INT);
					return JFuzzInterval.defaultHigh(nt);
				}
				else{
					NumericEndpoint lowEP = val.getLow();
